...
async function deploy () {  
  const ReceiverForwarderArtifacts = await ethers.getContractFactory('ReceiverForwarder')
  ...
  const targetOne = await deployTargetToken("TargetTokenOne","TONE",trustedForwarder); 
  const targetTwo = await deployTargetToken("TargetTokenTwo","TTWO",trustedForwarder);  
  const targetThree = await deployTargetToken("TargetTokenThree","TTHREE",trustedForwarder);   
